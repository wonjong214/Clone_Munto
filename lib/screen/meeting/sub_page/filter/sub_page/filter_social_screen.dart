...
class FilterSocialScreenState extends State<FilterSocialScreen> {
  ...
  AgeSlider ageSliderModel = new AgeSlider();
  RangeValues _rangeValues = const RangeValues(20, 50);

  FilterSocialScreenState() {}
  ...
  void init_locationList() {
    ...
    locationWidgetList = List.generate(
      koreaLocationList.length,
      (index) {
        return KoreaLocationContainer(
          value: index,
          groupValue: koreaLocationGroupValue,
          text: koreaLocationList[index],
            onChanged: (value) {
              setState(
                    () {
        ...
        child: Column(
          ...
          children: [
                  ...
                  setState(() {
                    if (value != RangeValues(20, 50)) {
                      if ((value.end.toInt() - value.start.toInt()) == 0)
                        ...
                        _rangeValues = value;
                      ageSliderModel.label =
                          '${_rangeValues.start.round().toString()}~${_rangeValues.end.round().toString()}세';
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: List.generate(
                  ageLabel.length, (index) => Text(ageLabel[index])),
            ),
            SizedBox(
              height: 35,
            ),
